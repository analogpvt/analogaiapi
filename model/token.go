...
func ValidateUserToken(key string) (token *Token, err error) {
	if key == "" {
		return nil, errors.New("not given token")
	}
	token = &Token{}
...
func PreConsumeTokenQuota(tokenId int, quota int) (err error) {
	...
	if quotaTooLow || noMoreQuota {
		go func() {
			...
			if err != nil {
				common.SysError("Failed to get user mailbox：" + err.Error())
			}
			prompt := "Your quota is almost exhausted"
			if noMoreQuota {
				prompt = "Your credit has been exhausted"
			}
			if email != "" {
				topUpLink := fmt.Sprintf("%s/topup", common.ServerAddress)
				err = common.SendEmail(prompt, email,
					fmt.Sprintf("%s，The current remaining balance is %d. In order not to affect your usage, please recharge in time. <br/>Recharge link：<a href='%s'>%s</a>", prompt, userQuota, topUpLink, topUpLink))
				if err != nil {
					common.SysError("Failed to send mail：" + err.Error())
				}
			}
		}()
	}
